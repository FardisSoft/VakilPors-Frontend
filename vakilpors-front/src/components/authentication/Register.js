...
const Register = () => {
  ...
  const descriptionUser =
    "به وکیل پرس خوش آمدید! اینجا می‌توانید هر سوال حقوقی که دارید را بپرسید! همینطور می‌توانید پرونده‌هایتان را قرار دهید تا وکیلی که می‌خواهید وکالت پرونده شما را قبول کند !";
  const descriptionLawyer =
    "به وکیل پرس خوش آمدید! اینجا میتوانید به سوالات حقوقی بقیه جواب دهید و امتیاز بگیرید! همچنین میتوانید پرونده های مختلف رو مشاهده کنید و وکالت پرونده دلخواه را قبول کنید!";
  ...
  const handleRoleChanger = () => {
    role === "user"
      ? setDescription(descriptionLawyer)
      : setDescription(descriptionUser);
    role === "user"
      ? setRoleTitle(roleTitleUser)
      : setRoleTitle(roleTitleLawyer);
  ...
  const validateForm = () => {
    ...
    if (
      ...
      showErrorMessage(
        "رمز خود را به صورت صحیح وارد کنید ( رمز شما باید بین 6 تا 30 کاراکتر باشد، فقط از اعداد و حروف انگلیسی تشکیل شده باشد و حداقل شامل یک حرف بزرگ و حداقل یک حرف کوچک باشد)"
      );
      return false;
    }
  ...
  const SignupApi = async () => {
    ...
    try {
      ...
      const url_2 =
        BASE_API_ROUTE +
        `Auth/sendactivationcode?phoneNumber=${phoneNumber.trim()}`;
      try {
      ...
      if (responseData.hasOwnProperty("data")) {
        if (responseData.data.hasOwnProperty("DuplicateUserName")) {
          showErrorMessage(
            "شما قبلا ثبت نام کرده اید! لطفا روی وارد شو کلیک کرده و وارد حساب کاربری خود شوید. "
          );
        } else {
          ...
          <div className="form-left">
            ...
            <label>
              <p className="text-1">
                ...
                <Link
                  to="/Login"
                  style={{ color: "lightskyblue", marginRight: "10px" }}
                >
                  وارد شوید!
                </Link>
              </p>
