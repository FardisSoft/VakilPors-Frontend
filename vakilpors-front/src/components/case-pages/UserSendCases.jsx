...
const UserSendCases = () => {
  ...
  useEffect(() => {
    const getUsers = async () => {
      const url =
        BASE_API_ROUTE +
        `Document/GetUsersThatLawyerHasAccessToTheirDocuments?lawyerId=${LawyerId}`;
          ...
          <CardHeader
            ...
            title={
              users.length == 0
                ...
                : "کاربرانی که برای شما پرونده ارسال کرده اند:"
            }
          />
        </Grid>
        {users.map((user, index) => (
          <Grid key={index} sx={{marginTop:'10px'}} container direction={{ xs: "column", sm: "row" }}>
            <Grid
              display="flex"
            ...
            >
              <CardContent>
                ...
                <Typography
                  onClick={() =>
                    navigate(`/show-cases/true_${user.id}_${LawyerId}`)
                  }
                  sx={{
                    fontFamily: "shabnam",
                    fontSize: "14px",
